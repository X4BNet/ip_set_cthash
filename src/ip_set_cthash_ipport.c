...
IP_SET_MODULE_DESC("cthash:ip,port", IPSET_TYPE_REV_MIN, IPSET_TYPE_REV_MAX);
MODULE_ALIAS("ip_set_cthash:ip,port");
		  ...
		  enum ipset_adt adt, struct ip_set_adt_opt *opt)
{
	...
	struct nf_conntrack_tuple *ctt = NULL;
	if(ct){
		ctt = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;
	} 

	if(opt->flags & IPSET_DIM_ONE_SRC){
		e.ip = ctt->src.u3.ip;
		e.port = ctt->src.u.tcp.port;
	}else{
		e.ip = ctt->dst.u3.ip;
		e.port = ctt->dst.u.tcp.port;
	}
	e.proto = ctt->dst.protonum;
